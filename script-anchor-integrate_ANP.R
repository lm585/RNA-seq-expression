...
bcell_all_Features <- lapply(splitLS, row.names) %>% Reduce(intersect, .))

bCombBcell.integrated.all <- IntegrateData(anchorset = splitAnchors, dims = 1:30, features.to.integrate = bcell_all_Features)
...
int_bcell_seurat_membership <- as.data.frame(bCombBcell.integrated@meta.data)["seurat_clusters"]

bCombBcell.integrated.all <- AddMetaData(bCombBcell.integrated.all, metadata = int_bcell_seurat_membership)

                        

bCombBcell.integrated.all <- ScaleData(bCombBcell.integrated,all, verbose = T)
bCombBcell.integrated.all <- RunPCA(bCombBcell.integrated.all, npcs = 30, verbose = T)
bCombBcell.integrated.all <- RunUMAP(bCombBcell.integrated.all, reduction = "pca", dims = 1:30)
